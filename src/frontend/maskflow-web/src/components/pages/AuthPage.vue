<template>
  <main v-if="page === 'auth'" class="auth-redesign">
    <section class="auth-hero-panel">
      ...
      <div><p class="home-badge">AI 自动标注平台</p><h1>把图片变成可训练数据集</h1><p>上传图片、自动分割、人工校正、导出 YOLO 数据集，所有流程都在一个清晰的工作台里完成。</p></div>
      <div class="auth-preview-mini"><div><span></span><b>AI 自动分割</b><em>38 / 50 次</em></div><div><span></span><b>项目图片</b><em>128 张</em></div><div><span></span><b>导出格式</b><em>YOLO txt</em></div></div>
    </section>
    <section class="auth-card">
      ...
      <form class="auth-form" @submit.prevent="submitAuth">
        ...
        <div class="form-row"><label>密码</label><input v-model="auth.password" type="password" autocomplete="current-password" placeholder="请输入密码" required /></div>
